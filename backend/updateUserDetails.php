...
if(isset($_POST['editUserDetails'])){
    ...
    $updateQuery = "UPDATE `divine_jwellery_payout_data` SET `Payout`=?,`contact`=?,`EmailId`=?,`AccountHolderName`=?,`AccountNumber`=?,`PaymentType`=?,`LastPaymentTransactionNo`=?,`LastPaymentTransactionDate`=?,`pay_to_id_1`=?,`pay_to_id_2`=?,`type_of_node`=?,`address`=?,`pincode`=?,`isactive`=?,`parentId`=?,`account_ifsc`=?,`account_branch_name`=?,`account_upi_number`=?,`passbook_link`=?,`aadhar_card_number`=?,`aadhar_card_name`=?,`aadhar_card_link`=?,`pan_card_number`=?,`pan_card_link`=?,`remark`=? WHERE `id` = ?";

    $update = $conn->prepare($updateQuery);
    $update->execute([$payout, $contact, $email, $name, $accNumber, $paymentType, $lastTrNo, $transcationDate, $payto1, $payto2, $typeofnode, $address, $pincode, $isActive, $parentId, $accountIFSC, $accountBranch, $accountUPINo, $passbooklink, $aadharcardno, $aadharcardname, $aadharcardlink, $pancardno, $pancardlink, $remark, $_SESSION["editUserID"]]);
